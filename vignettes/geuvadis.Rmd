---
title: "Example usage of zenith on RNA-seq"
subtitle: "Modeling correlation between genes"
author:
...
date: "`r date()`"
abstract: >
 Perform gene set analysis on the result of differential expression using linear (mixed) modeling with dream by considering the correlation between gene expression traits.  Zenith performs competitive gene sets analysis by comparing the tests statistics from genes in a  gene set to the rest of the genes not in the set.  This package is a slight modification of limma::camera to 1) be compatible with dream, and 2) allow identification of gene sets with log fold changes with mixed sign by using the squared test statistic.  
vignette: >
...
#### Load genesets
```{r load.genesets}
# Load Gene Ontology database using EnrichmentBrowser package
go.gs <- getGenesets(org="hsa", db="go", return.type='GeneSetCollection')

# Convert gene identifiers from the default Entrez to Ensembl
go.gs = idMap(go.gs, org = "hsa", from = "ENTREZID", to = "ENSEMBL") 

# modify the names of the gene sets to read
# "GO0000002: mitochondrial genome maintenance"
# instead of "GO0000002"
go.gs_mod = lapply( go.gs, function(gs){
   setName(gs) = paste0( setName(gs), ': ', description(gs))
   gs
  } )
go.gs_mod = GeneSetCollection(go.gs_mod )

# convert from GeneSetCollection to list used by camera and zenith
geneSets_GO = recodeToList( go.gs_mod )
```
...
res_zenith2[1:8,] %>% kable %>% kable_styling(full_width = FALSE)
```
...
res_zenith_msigdb[1:8,] %>% kable %>% kable_styling(full_width = FALSE)
```
